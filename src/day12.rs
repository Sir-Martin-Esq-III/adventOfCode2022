...
fn successor((x, y): &(i32, i32), grid: &Vec<Vec<u8>>) -> Vec<(i32, i32)> {
    ...
    if within_bounds(&y, &0, &(grid.len() as i32 - 1)) {
        if grid[(y - 1) as usize][*x as usize] <= current_point_value + 1 {
            points.push((*x, y - 1))
        }
    }
    ...
    if within_bounds(&x, &0, &(grid[0].len() as i32 - 1)) {
        if grid[*y as usize][(x - 1) as usize] <= current_point_value + 1 {
            points.push((x - 1, *y))
        }
    }
...
pub fn solve_part1(input: &(Vec<Vec<u8>>, (i32, i32), (i32, i32))) -> usize {
    let res = dijkstra(
        &input.1,
        move |f| {
            return successor(f, &input.0).into_iter().map(|p| (p, 1));
        },
        |f| *f == input.2,
    );

    return res.unwrap().0.len() - 1;
}
